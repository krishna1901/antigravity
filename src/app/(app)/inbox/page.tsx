...
export default function InboxPage() {
  ...
  const positiveCount = inboxThreads.filter((t) => t.sentiment === "positive").length;
  const repliedCount = inboxThreads.filter((t) => t.status === "replied").length;
  const positivePct = Math.round((positiveCount / inboxThreads.length) * 100);
              ...
              <footer className="space-y-3 border-t border-border bg-muted/20 p-4">
                <div className="flex items-end gap-2">
                  <Textarea
                    ...
                    rows={1}
                  />
                  <Button variant="gradient" className="h-11 shrink-0" disabled={!reply.trim()}>
